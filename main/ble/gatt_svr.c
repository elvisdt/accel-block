...
static const char *manuf_name = "HUNTER";
static const char *model_num = "MKEY";
...
static int gatt_svr_chr_dac_cfg_cb(uint16_t conn_handle, uint16_t attr_handle,
                                   struct ble_gatt_access_ctxt *ctxt,
                                   void *arg);
...
static const struct ble_gatt_svc_def gatt_svr_svcs[] = {
...

    {
        ...
        .characteristics =
            (struct ble_gatt_chr_def[]){
                {
                    // characteristic: DAC config (u1_mV LE, u2_mV LE)
                    .uuid = &gatt_svr_chr_dac_cfg_uuid.u,
                    .access_cb = gatt_svr_chr_dac_cfg_cb,
                    .flags = BLE_GATT_CHR_F_READ | BLE_GATT_CHR_F_WRITE,
                    .val_handle = &dac_cfg_val_handle,
                },
                {
                    0,
                }},
...
static int gatt_svr_chr_dac_cfg_cb(uint16_t conn_handle, uint16_t attr_handle,
  ...
  switch (ctxt->op) {
    case BLE_GATT_ACCESS_OP_READ_CHR: {
      app_get_dac_voltage(&u1, &u2);
      uint16_t u1_mv = (uint16_t)(u1 * 1000.0f);
      uint16_t u2_mv = (uint16_t)(u2 * 1000.0f);
      uint8_t payload[4] = {
          (uint8_t)(u1_mv & 0xFF),
          (uint8_t)((u1_mv >> 8) & 0xFF),
          (uint8_t)(u2_mv & 0xFF),
          (uint8_t)((u2_mv >> 8) & 0xFF),
      };
      rc = os_mbuf_append(ctxt->om, payload, sizeof(payload));
      return rc == 0 ? 0 : BLE_ATT_ERR_INSUFFICIENT_RES;
    }
    ...
    case BLE_GATT_ACCESS_OP_WRITE_CHR: {
      uint8_t payload[4] = {0};
      uint16_t out_len = 0;
      rc = gatt_svr_chr_write(ctxt->om, sizeof(payload), sizeof(payload), payload,
                              &out_len);
      if (rc != 0) {
        return rc;
      }

      uint16_t u1_mv = (uint16_t)(payload[1] << 8 | payload[0]);
      uint16_t u2_mv = (uint16_t)(payload[3] << 8 | payload[2]);
      u1 = ((float)u1_mv) / 1000.0f;
      u2 = ((float)u2_mv) / 1000.0f;
      app_set_dac_voltage(u1, u2);
      return 0;
    }
